...
from .models import Comments, ProductInShop, Specifications, Subspecifications
from .services import filter_catalog, get_maxprice, paginator, sort_catalog
...
class ProductInShopDetailView(DetailView, FormMixin):
    queryset = (
        ProductInShop.objects.select_related("product")
    ...
    def get_context_data(self, **kwargs):
        ...
        subspecifications = Subspecifications.objects.filter(
            specification__in=specifications
        ).all()
            ...
            if text_comment:
                if user == AnonymousUser():
                    messages.add_message(
                        request,
                        messages.ERROR,
                        "Для оставления отзыва нужно зарегистрироваться",
                    )
                else:
                    product_in_shop = ProductInShop.objects.get(pk=pk)
                    new_comment = Comments.objects.create(
                        user=user, comment=text_comment, product_in_shop=product_in_shop
                    )
                    new_comment.save()
