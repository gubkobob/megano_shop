...
class CheckoutOrderView(View):
    def get(self, request, *args, **kwargs):
        ...
        context = {"form": form, "settings_price": settings_price}
    ...
    def post(self, request, *args, **kwargs):
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                get_total_price_before = sum(
                    Decimal(item.product_in_shop.price) * item.quantity
                    for item in order_items
                )
                get_total_price = sum(
                    Decimal(item.price) * item.quantity for item in order_items
                )

                if new_order.delivery == "Экспресс доставка":
                    new_order.delivery_cost = getattr(
                        SettingsModel.objects.first(), "price_express_delivery"
                    )
                elif (
                    get_total_price
                    < getattr(SettingsModel.objects.first(), "min_total_price_order")
                ) or (len(self.count_shop) > 1):
                    new_order.delivery_cost = getattr(
                        SettingsModel.objects.first(), "price_ordinary_delivery"
                    )
